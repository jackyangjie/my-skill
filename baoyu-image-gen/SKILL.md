...
Official API-based image generation. Supports OpenAI, Google, DashScope (阿里通义万象), Replicate, and Zhipu (智谱 AI) providers.
...
| `--provider google\|openai\|dashscope\|replicate\|zhipu` | Force provider (default: google) |
...
2. `--provider` specified → use it (if `--ref`, must be `google`, `openai`, `replicate`, or `zhipu`)
