...
Options:
  ...
  --provider google|openai|dashscope|replicate|zhipu  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  ZHIPU_API_KEY             Zhipu API key (智谱 AI)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "replicate" && v !== "zhipu") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, replicate: null, zhipu: null };
        currentKey = "default_model";
      } else if (currentKey === "default_model" && (key === "google" || key === "openai" || key === "dashscope" || key === "replicate" || key === "zhipu")) {
        const cleaned = value.replace(/['"]/g, "");
...
function detectProvider(args: CliArgs): Provider {
  ...
  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasReplicate && "replicate", hasZhipu && "zhipu"].filter(Boolean) as Provider[];

  if (available.length === 1) return available[0]!;
  if (available.length > 1) return available[0]!;

  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, REPLICATE_API_TOKEN, or ZHIPU_API_KEY.\n" +
...
async function main(): Promise<void> {
  ...
  if (!model && extendConfig.default_model) {
    ...
    if (provider === "zhipu") model = extendConfig.default_model.zhipu ?? null;
  }
